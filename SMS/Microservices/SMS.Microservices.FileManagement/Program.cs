using SMS.Microservices.FileManagement.MappingProfiles;
using Polly;
using MassTransit;
...
builder.Services.AddAutoMapper(typeof(FileManagementMappingProfile).Assembly);
...
{
    options.UseNpgsql(builder.Configuration.GetConnectionString("FileManagementDbConnection"));
});
